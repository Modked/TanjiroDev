let istighfarList = [
    ...
    "*أستغفر الله العظيم* من كل محمود سئمتـــه",
    "*أستغفر الله العظيم* من كل زور نطقـت بـه",
    ...
    "*أستغفر الله العظيم* من كل صواب كتمتــه",
    "*أستغفر الله العظيم* من كل خطأ تفوهـت بـه",
    "*أستغفر الله العظيم* من كل عرض هتكتــه",
    "*أستغفر الله العظيم* من كل ستر فضحتــه",
    "*أستغفر الله العظيم* من كل لغو سمعتــه",
    "*أستغفر الله العظيم* من كل حرام نظرت إليـه",
    "*أستغفر الله العظيم* من كل كلام لهوت بــه",
    "*أستغفر الله العظيم* من كل إثـم فعلتـــه",
    "*أستغفر الله العظيم* من كل نصح خالفتــه",
    "*أستغفر الله العظيم* من كل علم نسيتــه",
    "*أستغفر الله العظيم* من كل شك أطعتــه",
    "*أستغفر الله العظيم* من كل ظن لازمتــه",
    "*أستغفر الله العظيم* من كل ضلال عرفتــه",
    "*أستغفر الله العظيم* من كل دين أهملتــه",
    "*أستغفر الله العظيم* من كل معصية عصيت بها ربي",
    "*أستغفر الله العظيم* من كل ما وعدتك به ثم عدت فيه من نفسي ولم أوف به",
    ...
    "*أستغفر الله العظيم* من كل قول صالح لم أعمل به وخالفتــه",
    "*أستغفر الله العظيم* من كل فرض خالفته ومن كل بدعة إتبعتهــا",
...
function handler(m) {
    if (currentIndex < istighfarList.length) {
        m.reply(istighfarList[currentIndex]);
        currentIndex++;
    } else {
        m.reply("تم إرسال جميع الاستغفارات.");
        currentIndex = 0; // إعادة التعيين للبدء من جديد
    }
}
