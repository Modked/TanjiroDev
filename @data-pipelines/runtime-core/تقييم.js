...
const handler = async (message, { conn, text, usedPrefix }) => {
    ...
    if (deviceType !== "desktop" && deviceType !== "web") {
        ...
        const mediaMessage = await prepareWAMessageMedia({
            image: { url: "https://files.catbox.moe/t2ie1w.jpg" }
...
const sendFeedbackResponse = (stars, m, conn) => {
    let feedbackMessage = '';
    let developerContact = '967772350066@s.whatsapp.net'; // رقم المطور للتواصل
    ...
    // رد البوت للمستخدم
    m.reply(feedbackMessage + `\n\n*رابط قناة ساسكي بوت الرسمية،ادخل تنورنا فيها:* wa.me/${developerContact.split('@')[0]}`);
};
