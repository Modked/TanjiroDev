    ...
    try {
      ...
      const { title, videoUrl, thumbnail } = playmp3;
           ...
           await conn.sendMessage(m.chat, { react: { text: '✔️', key: m.key } });
             ...
             await conn.sendMessage(m.chat, {video: {url: videoUrl}, gifPlayback: true, fileName: title + '.gif', caption: `⛊ العنوان : ${title}\n`}, {quoted: m});
             
             
          } catch {
          
            await conn.reply(m.chat, '*❲ ❗ ❳ حدث خطأ عند جلب ملف الجيف.*', m);
          }
        
      
      
        } catch {
      await conn.sendMessage(m.chat, { text: `*❲ ❗ ❳ حدث خطأ عند البحث عن ملف الجيف .*\nيرجي ادخال نص صحيح أو رابط مثال :\n> ➤  ${usedPrefix + command} القرآن الكريم\n> ➤  ${usedPrefix + command} https://youtu.be/JLWRZ8eWyZo?si=EmeS9fJvS_OkDk7p` }, { quoted: m });
      await conn.sendMessage(m.chat, { react: { text: '❌', key: m.key } });
    }
