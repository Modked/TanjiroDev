    ...
    try {
      ...
      const { title, videoUrl, thumbnail } = playmp3;
           ...
           await conn.sendMessage(m.chat, { react: { text: '✔️', key: m.key } });
             ...
             conn.sendMessage(m.chat, {audio: {url: videoUrl}, mimetype: 'audio/ogg; codecs=opus', ptt: true, fileName: title + '.opus'}, {quoted: m});
             
          } catch {
          
            await conn.reply(m.chat, '*❲ ❗ ❳ حدث خطأ عند جلب الريك.*', m);
          }
        
      
      
        } catch {
      await conn.sendMessage(m.chat, { text: `*❲ ❗ ❳ حدث خطأ عند البحث عن الريك .*\nيرجي ادخال نص صحيح أو رابط مثال :\n> ➤  ${usedPrefix + command} القرآن الكريم\n> ➤  ${usedPrefix + command} https://youtu.be/JLWRZ8eWyZo?si=EmeS9fJvS_OkDk7p` }, { quoted: m });
      await conn.sendMessage(m.chat, { react: { text: '❌', key: m.key } });
    }
