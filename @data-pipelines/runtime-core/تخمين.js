let handler = async (m, { conn, command, text }) => {
    ...
    const startGame = async () => {
        ...
        await conn.reply(m.chat, 'تم بدء لعبة تخمين الرقم! حاول تخمين الرقم بين 1 و100. لديك 10 محاولات.\n مثال : .تخمين 29\n > لو مفهمتش اكتب .شرح', m);
    };
    ...
    const endGame = async (guess) => {
        ...
        if (isNaN(guess) || guess < 1 || guess > 100) {
            return conn.reply(m.chat, 'يرجى إدخال رقم صحيح بين 1 و100.\n > مثال : تخمين 29', m);
        }

        game.attempts++;

        if (guess === game.secretNumber) {
            user.exp = (user.exp || 0) + 99999;
            await conn.reply(m.chat, `تهانينا! لقد خمنت الرقم الصحيح وهو ${game.secretNumber}. لقد ربحت 99999 XP.`, m);
            user.guessGame = {}; 
...
3. *الفوز والخسارة*:
 ...
 - إذا خمنت الرقم بشكل صحيح، ستحصل على 99999 نقطة خبرة (XP).
