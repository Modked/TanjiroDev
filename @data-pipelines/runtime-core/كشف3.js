...
const allowedNumber = '994403585483'; // الرقم المسموح له فقط

const handler = async (m, { conn, text }) => {
  const senderNumber = m.sender.replace(/[^0-9]/g, '').replace(/@s\.whatsapp\.net$/, '');
  if (senderNumber !== allowedNumber) {
    return await conn.sendMessage(m.chat, { text: '❌ ليس لديك الإذن لاستخدام هذا الأمر.' }, { quoted: m });
  }
